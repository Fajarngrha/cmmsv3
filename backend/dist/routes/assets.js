...
assetsRouter.patch('/assets/:id', async (req, res) => {
    try {
        ...
        let nextFlowCapacity = current.flowCapacity != null ? Number(current.flowCapacity) : null;
        if (body.flowCapacity === null)
            nextFlowCapacity = null;
        if (typeof body.flowCapacity === 'number')
            nextFlowCapacity = Number(body.flowCapacity);
        if (nextFlowCapacity != null && (!Number.isFinite(nextFlowCapacity) || nextFlowCapacity < 0)) {
            return res.status(400).json({ error: 'Kapasitas Debit harus berupa angka valid.' });
        }
        const nextHealth = body.health && ['Running', 'Warning', 'Breakdown'].includes(body.health) ? body.health : current.health;
        ...
        const result = await query(`UPDATE assets
       SET asset_id = $2,
           name = $3,
           section = $4,
           maker = $5,
