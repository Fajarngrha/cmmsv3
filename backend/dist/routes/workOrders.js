...
workOrdersRouter.patch('/work-orders/:id', (req, res) => {
    ...
    if (body.status === 'Completed' && wo.status === 'In Progress') {
        ...
        if (body.causeOfDamage != null)
            wo.causeOfDamage = body.causeOfDamage;
        if (body.repairsPerformed != null)
            wo.repairsPerformed = body.repairsPerformed;
        if (body.actionType != null)
            wo.actionType = body.actionType;
        if (body.replacedSpareParts != null)
            wo.replacedSpareParts = body.replacedSpareParts;
        if (body.replacedPartsSpec != null)
            wo.replacedPartsSpec = body.replacedPartsSpec;
        if (body.replacedPartsQty != null)
            wo.replacedPartsQty = body.replacedPartsQty;
        if (body.technician != null)
            wo.technician = body.technician;
        return res.json(wo);
    }
