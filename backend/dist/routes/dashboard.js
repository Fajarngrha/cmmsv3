...
dashboardRouter.get('/dashboard/kpis', (_, res) => {
    ...
    const totalMaintenanceCost = mock.purchaseOrders.reduce((sum, po) => sum + (typeof po.totalHarga === 'number' ? po.totalHarga : 0), 0);
...
dashboardRouter.patch('/dashboard/upcoming-pm/:id', (req, res) => {
    ...
    if (body.keteranganStatus !== undefined) {
        ...
        if (v === 'PM OK' || v === 'Belum Selesai' || v === 'Pending')
            pm.keteranganStatus = v;
    }
    if (body.keteranganNotes !== undefined)
        pm.keteranganNotes = String(body.keteranganNotes).trim() || undefined;
    res.json(pm);
});
