...
permintaanPerbaikanRouter.get('/permintaan-perbaikan/:id', async (req, res) => {
    try {
        ...
        if (result.rows.length === 0)
            return res.status(404).json({ error: 'Permintaan perbaikan tidak ditemukan' });
...
permintaanPerbaikanRouter.patch('/permintaan-perbaikan/:id', async (req, res) => {
    try {
        ...
        const body = req.body;
        if (body.status === 'Open') {
            await query('UPDATE permintaan_perbaikan SET status = $1, started_at = NULL, pending_reason = NULL WHERE id = $2', ['Open', id]);
        ...
        if (body.status === 'In Progress' && (row.status === 'Open' || row.status === 'Pending')) {
            await query('UPDATE permintaan_perbaikan SET status = $1, started_at = CURRENT_TIMESTAMP, pending_reason = NULL WHERE id = $2', ['In Progress', id]);
            const r = await query('SELECT * FROM permintaan_perbaikan WHERE id = $1', [id]);
            return res.json(rowToWorkOrder(r.rows[0]));
        }
        if (body.status === 'PM') {
            ...
            if (!pmDate)
                return res.status(400).json({ error: 'Tanggal PM wajib diisi.' });
            ...
            await query('INSERT INTO upcoming_pm (pm_id, asset_name, activity, scheduled_date, assigned_to) VALUES ($1, $2, $3, $4, $5)', [nextPmId, row.machine_name, row.damage_type || 'Preventive', pmDate, row.technician || '—']);
            await query('UPDATE permintaan_perbaikan SET status = $1, type = $2, pm_scheduled_date = $3 WHERE id = $4', ['PM', 'PM', pmDate, id]);
            const r = await query('SELECT * FROM permintaan_perbaikan WHERE id = $1', [id]);
            return res.json(rowToWorkOrder(r.rows[0]));
        }
        if (body.status === 'Pending') {
            ...
            if (!reason)
                return res.status(400).json({ error: 'Alasan pending wajib diisi.' });
        ...
        if (body.status === 'Completed' && row.status === 'In Progress') {
            ...
            const started = row.started_at ? new Date(row.started_at).getTime() : closedAt.getTime();
            const totalDowntimeHours = Math.round(((closedAt.getTime() - started) / (1000 * 60 * 60)) * 100) / 100;
            await query(`UPDATE permintaan_perbaikan SET status = $1, closed_at = $2, total_downtime_hours = $3,
...
permintaanPerbaikanRouter.post('/permintaan-perbaikan', async (req, res) => {
    try {
        ...
        const result = await query(`INSERT INTO permintaan_perbaikan (wo_id, machine_name, machine_brand, section, machine_status, damage_type, status, due_date, reported_by)
