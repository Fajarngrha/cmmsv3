...
export type TokenPayload = { sub: string; name?: string; iat: number; exp: number }

const TTL_MS = 7 * 24 * 60 * 60 * 1000

export function signToken(username: string, secret: string, displayName?: string | null): string {
...
export function verifyToken(
  token: string,
  secret: string
): { ok: true; username: string; displayName?: string } | { ok: false } {
  ...
  try {
    ...
    if (typeof payload.sub !== 'string' || typeof payload.exp !== 'number') return { ok: false }
