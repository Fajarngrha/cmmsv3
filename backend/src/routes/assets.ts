...
assetsRouter.get('/assets/:assetId/history', async (req, res) => {
  try {
    ...
    if (!inputAssetId) return res.status(400).json({ error: 'assetId wajib diisi.' })

    const assetResult = await query('SELECT * FROM assets WHERE LOWER(asset_id) = LOWER($1) LIMIT 1', [inputAssetId])
    if (assetResult.rows.length === 0) return res.status(404).json({ error: 'Asset tidak ditemukan.' })
    ...
    const historyResult = await query(
      ...
      [machineNameKey, machineAssetIdKey]
    )
