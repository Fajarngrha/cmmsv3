...
function rowToPo(row: PurchaseOrderRow) {
  return {
    id: String(row.id),
    ...
    kategori: row.kategori as POKategori,
    totalHarga: Number(row.total_harga),
    status: row.status as POStatus,
  }
}

async function nextNoRegistrasiFromDb(): Promise<string> {
  const now = new Date()
...
function nextNoRegistrasiMock(): string {
  const now = new Date()
  const mm = String(now.getMonth() + 1).padStart(2, '0')
  const yy = String(now.getFullYear()).slice(-2)
  const prefix = `MTC/SPB/${mm}/${yy}/`
...
purchaseOrdersRouter.patch('/purchase-orders/:id', async (req, res) => {
  ...
  try {
    if (isDbConfigured()) {
      ...
      if (updates.length === 0) {
        const r = await query<PurchaseOrderRow>(
          `SELECT id, tanggal, item_deskripsi, model, harga_per_unit, qty, no_registrasi, no_po, mesin, no_quotation, supplier, kategori, total_harga, status FROM purchase_orders WHERE id = $1`,
          [id]
        )
