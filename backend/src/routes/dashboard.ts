...
dashboardRouter.get('/dashboard/kpis', async (_, res) => {
  try {
    const [costRow, woRows, pmRows, assetRows] = await Promise.all([
      query<{ sum: string }>('SELECT COALESCE(SUM(total_harga), 0)::text AS sum FROM purchase_orders'),
      query<{ status: string; c: string }>(
        'SELECT status, COUNT(*)::text AS c FROM permintaan_perbaikan GROUP BY status'
      ),
      query('SELECT id, keterangan_status FROM upcoming_pm'),
      query<{ health: string; c: string }>('SELECT health, COUNT(*)::text AS c FROM assets GROUP BY health'),
    ])
    const totalMaintenanceCost = Number(costRow.rows[0]?.sum ?? 0)
    const woCountByStatus: Record<string, number> = {}
    woRows.rows.forEach((r) => { woCountByStatus[r.status] = parseInt(r.c, 10) })
    const openWorkOrders =
      (woCountByStatus['Open'] ?? 0) + (woCountByStatus['Pending'] ?? 0) + (woCountByStatus['PM'] ?? 0)
    const assetsInMaintenance = woCountByStatus['In Progress'] ?? 0
    const totalPM = pmRows.rows.length
    const compliantCount = pmRows.rows.filter((r) => (r as { keterangan_status?: string }).keterangan_status === 'PM OK').length
...
dashboardRouter.get('/dashboard/trend', async (_, res) => {
  try {
    const result = await query<{ ym: string; month: string; type: string; c: string }>(
    ...
    const byMonth: Record<string, { month: string; reactiveWOs: number; preventiveWOs: number }> = {}
    result.rows.forEach((r) => {
      if (!byMonth[r.ym]) byMonth[r.ym] = { month: r.month, reactiveWOs: 0, preventiveWOs: 0 }
      const count = parseInt(r.c, 10)
      if (r.type === 'PM') byMonth[r.ym].preventiveWOs += count
      else byMonth[r.ym].reactiveWOs += count
    })
...
dashboardRouter.get('/dashboard/pareto', async (_, res) => {
  try {
    const result = await query<{ cause_of_damage: string; hours: string }>(
      `SELECT COALESCE(LEFT(cause_of_damage, 50), 'Lainnya') AS cause_of_damage,
...
dashboardRouter.patch('/dashboard/upcoming-pm/:id', async (req, res) => {
  try {
    ...
    const body = req.body as { keteranganStatus?: string; keteranganNotes?: string }
    ...
    if (updates.length === 0) {
      ...
      if (r.rows.length === 0) return res.status(404).json({ error: 'Jadwal PM tidak ditemukan.' })
...
dashboardRouter.delete('/dashboard/upcoming-pm/:id', async (req, res) => {
  try {
    ...
    if (result.rowCount === 0) return res.status(404).json({ error: 'Jadwal PM tidak ditemukan.' })
    res.status(204).send()
  } catch (e) {
    console.error(e)
...
dashboardRouter.post('/dashboard/pm-schedule', async (req, res) => {
  try {
    ...
    const result = await query(
      `INSERT INTO upcoming_pm (pm_id, asset_name, activity, scheduled_date, assigned_to,
...
dashboardRouter.get('/dashboard/quick-stats', async (_, res) => {
  try {
    const completed = await query<{ c: string }>(
      "SELECT COUNT(*)::text AS c FROM permintaan_perbaikan WHERE status = 'Completed'"
    )
    res.json({
...
dashboardRouter.get('/dashboard/wo-status', async (_, res) => {
  try {
    const result = await query<{ status: string; c: string }>(
      'SELECT status, COUNT(*)::text AS c FROM permintaan_perbaikan GROUP BY status'
    )
...
dashboardRouter.get('/dashboard/asset-health', async (_, res) => {
  try {
    const result = await query<{ health: string; c: string }>('SELECT health, COUNT(*)::text AS c FROM assets GROUP BY health')
    const m: Record<string, number> = {}
